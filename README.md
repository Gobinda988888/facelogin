...
This repository includes `render.yaml`, so Render can configure the service automatically:

1. Open the [Render Dashboard](https://dashboard.render.com/) and select **New +** → **Blueprint**.
2. Connect the `Gobinda988888/facelogin` repository.
3. Confirm the service settings from `render.yaml` and click **Apply**.
4. After deployment, open the generated Render URL over HTTPS to allow camera access.

The service uses Gunicorn and binds to Render's `$PORT` automatically. `SECRET_KEY` is generated by Render.
</details>
